...
# Order to show the values
order <- levels(toplot$k3mer)[order(sapply(k, function(x) unlist(strsplit(x,split=""))[2]))]
toplot$k3mer <- factor(toplot$k3mer, levels = order)

# Show the relative values in %percentage
toplot$Rel <- toplot$Rel *100

boxplot <- ggplot(toplot, aes(y=Rel, x=k3mer, fill=Region)) + 
  geom_boxplot() + 
  labs(x = "k3-mer sequence", y = "Percentage in reference genome") +
